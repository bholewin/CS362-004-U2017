...
int main () {
  ...
  printf("*************************************************\n");
  printf("This test will check the great_hall card.\n");
  int currentPlayer = whoseTurn(state);
  int temphand[MAX_HAND];
  card = handCard(0, currentG);
  cardEffect(card, 0, 0, 0, &currentG, 0, 0);
  assertTrue(OG.handCount[0] == currentG.handCount[0], "Player gains a card and discards their great_hall card.");
  ...
  assertTrue(OG.numActions + 1 == currentG.numActions, "Player gained one action.");
  ...
  assertTrue(OG.coins - 3 == currentG.coins, "Coin amount was properly calculated.");
