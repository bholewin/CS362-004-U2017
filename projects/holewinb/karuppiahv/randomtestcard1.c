...
int great_hallCardTest(int p, struct gameState *post, int handPos)
{
  ...
  callStatus = cardEffect(card, 0, 0, 0, post, 0, 0);
  assert(callStatus == 0);
...
int main()
{
  ...
  for (n = 0; n < 15000; n++)
  {
    ...
    great_hallCardTest(p, &G, handPos);
  }
  printf("End of great_hall testing.\n");
  return 0;
}
