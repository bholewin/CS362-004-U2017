...
//This random test will go over the smithy card
//The adventurer card has a bug where the player draws an extra treasure card
int smithyCardTest(int p, struct gameState *post, int handPos)
{
  ...
  callStatus = Smithy(currentPlayer, post, handPos);
  assert(callStatus == 0);
  ...
  //need to alter due to bug introduced
...
int main()
{
  ...
  printf("*************************************************\n");
  printf("This test will check the smithy card.\n");
  for (n = 0; n < 15000; n++)
  {
    ...
    //select a card position and set to smithy
    int handPos = floor(Random() * G.handCount[p]);
    G.hand[p][handPos] = smithy;
    smithyCardTest(p, &G, handPos);
  }
  printf("End of smithy testing.\n");
  return 0;
}
